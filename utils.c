...
static void utils_yuv420p_to_422(int * out,unsigned char *pic,int width);
static void utils_yuv422p_to_422(int * out,unsigned char *pic,int width);
static void utils_yuv444p_to_422(int * out,unsigned char *pic,int width);
static void utils_yuv400p_to_422(int * out,unsigned char *pic,int width);
...
*/    
   ...
   // printf("find 422 %dx%d\n",*width,*height);
	...
	ypitch = 8 * pitch;
	 if (info.ns==1) {
    		mb = 1;
		convert = utils_yuv400p_to_422;
	} else {
		mb=3;
		convert = utils_yuv444p_to_422;	
	}
        break;
    default:
	err = ERR_NOT_YCBCR_221111;
...
unsigned int
utils_yuv422p_to_rgb24(unsigned char *input_ptr, unsigned char * output_ptr, unsigned int image_width, unsigned int image_height)
{
	unsigned int i, size;
...
static void utils_yuv400p_to_422(int32_t *out,unsigned char *pic, int32_t width)
{
    int j, k;
    unsigned char *pic0, *pic1;
    int *outy ;
...
int32_t utils_is_huffman(unsigned char *buf)
{
    unsigned char *ptbuf;
    int i = 0;
    ptbuf = buf;
...
void utils_get_picture_name (char *picture, const char *name_prefix, int32_t fmt)
{
    ...
    time_t curdate;
    struct tm *tdate;
    const char *tmp_prefix = NULL;
    int32_t tmp_len = 0;
    memset (temp, '\0', sizeof (temp));
    time (&curdate);
    tdate = localtime (&curdate);
    tmp_len = strlen(name_prefix);
    if (tmp_len == 0 || tmp_len > 20) {
        tmp_prefix = "P";
        tmp_len = 1;
    } else {
        tmp_prefix = name_prefix;
    }
    snprintf (temp, 25 + tmp_len, "%s-%02d_%02d_%04d-%02d_%02d_%02d.%s", tmp_prefix,
...
int utils_get_picture_mjpg(const char *name_prefix, unsigned char *buf, int32_t size)
{
    FILE *file;
    ...
    name = calloc(80,1);
    utils_get_picture_name(name, name_prefix, 1);
    file = fopen(name, "wb");
    if (file != NULL) {
...
int utils_get_picture_yv2(const char *name_prefix, unsigned char *buf, int32_t width, int32_t height)
{
    ...
    name = calloc(80,1);
    utils_get_picture_name(name, name_prefix, 0);
    picture = (unsigned char *)malloc(width * height * 3 * sizeof(char));
    if (picture) {
	    utils_yuv422p_to_rgb24(buf, picture, width, height);
    } else {
	    printf(" no room to take a picture \n");
	    return 0;
    }
